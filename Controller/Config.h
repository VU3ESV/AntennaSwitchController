...
#define CFG_MAGIC    0x414E5433UL  // "ANT3" — v3: adds SO2R mode/peer/interlock
#define EEPROM_SIZE  512

// Radio transport for this unit's radio source.
enum RadioType : uint8_t { RADIO_TCI = 0, RADIO_FLEX = 1 };

// SO2R role of this unit (docs/MULTI-RADIO-SO2R-PLAN.md §3).
//   STANDALONE — today's single-radio 8×1 (default).
//   MASTER     — Mode A Radio-1 board; the LAN interlock arbiter.
//   SLAVE      — Mode A Radio-2 board; claims antennas from the master.
// (DUAL, the single-board Mode B, is reserved for P2a — not handled here.)
...
// v3 settings. tci_host/tci_port are the radio endpoint for whichever transport
...
struct Config {
  ...
  uint8_t  mode;                 // CtrlMode: 0=standalone,1=master,2=slave
  char     peer_host[64];        // SLAVE: master's address (e.g. ANT-SW-…-1F.local)
...
// Returns true if a valid config was loaded (incl. a migrated v1/v2); false if
...
inline bool configLoad(Config& c) {
  ...
  if (configMigrateV2(c) || configMigrateV1(c)) {  // older image → upgrade
    clampConfig(c);
    configSave(c);                                 // persist as v3 (migrate once)
    return true;
  }
  configDefaults(c);
  return false;
